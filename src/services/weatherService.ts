...
export const getWeather = async (
    cityName: string,
    signal?: AbortSignal
): Promise<ReturnType<typeof transformWeatherData> | null> => {
  try {
    const cityData = await geo.getGeoData(cityName, 5, API_KEY, signal);

    if (!cityData?.data?.length) throw new Error("Город не найден.");

    const toLowerCaseCityName = cityName.toLowerCase();

    const currentCity: CityData | undefined = cityData.data.find(
        (city: CityData) => {
          if (city.name.toLowerCase() === toLowerCaseCityName) return true;
          if (!city?.local_names) return false;

          return Object.values(city.local_names)
              .some(name => name.toLowerCase() === toLowerCaseCityName);
        }
    );
